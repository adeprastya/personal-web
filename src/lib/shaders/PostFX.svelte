<script lang="ts">
  ...
  import { BloomEffect, EffectComposer, EffectPass, RenderPass, NoiseEffect, BlendFunction } from 'postprocessing'
  ...
  const bloomEffect = untrack(() => new BloomEffect({
  ...
  const noiseEffect = untrack(() => new NoiseEffect({
    blendFunction: BlendFunction.AVERAGE,
    premultiply: true
  }))
  const chromaticEffect = untrack(() => new ChromaticAberrationEffect({
    strength: 0.006,
    blendFunction: BlendFunction.SCREEN
  }))
  const dimensionalEffect = untrack(() => new InvertedDimensionalEffect({
    progress: 0,
    time: 0,
    autoTime: true
  }))
  ...
  $effect(() => {
    const active = activeProjectData.isVisible;

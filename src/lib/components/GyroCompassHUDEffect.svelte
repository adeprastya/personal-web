...
<div class="relative flex w-[80vw] sm:w-[60vw] items-center gap-2 font-mono text-[rgba(230,236,242,0.78)] select-none">
	<!-- Bracket -->
	<span class="absolute -top-2 -left-3 text-base leading-none text-white/40">[</span>
	<span class="absolute -top-2 -right-3 text-base leading-none text-white/40">]</span>

	<!-- Main ruler track -->
	<div bind:this={trackEl} class="relative h-7 flex-1 overflow-hidden">
		<!-- Baseline -->
		<div class="absolute top-0 right-0 left-0 h-px bg-white/30"></div>
		...
		>
			{#each ticks as t (t.i)}
				<div class="flex flex-col items-center">
					<span class={t.major ? 'h-2 w-px bg-white/40' : 'h-1 w-px translate-y-2 bg-white/20'}
					></span>
					{#if t.major}
						<span class="mt-[3px] text-[8px] tracking-[0.04em] text-white/35">
							{t.label}
						</span>
					{/if}
				</div>
		...
		<!-- Diamond marker -->
		<div bind:this={markerEl} class="absolute top-1/2 left-1/2 -mt-1 -ml-1 will-change-transform">
			<div
				class="h-2 w-2 rotate-45 bg-[rgba(230,236,242,0.95)] [filter:drop-shadow(0_0_3px_rgba(150,220,255,0.6))]"
			></div>
		</div>
	</div>
</div>

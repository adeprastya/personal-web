...
// ============ SHADER TEMPLATES ============
const VERTEX_INJECTION = `varying vec3 vWorldPos;
`
const VERTEX_REPLACE = `#include <worldpos_vertex>
vWorldPos = worldPosition.xyz;`

const FRAGMENT_INJECTION = `varying vec3 vWorldPos;
...
`
const FRAGMENT_REPLACE = `if(uActive > 0.5){
...
function injectLaser(material: THREE.Material) {
  if ((material as any).__laserInjected) return
  ...
  ;(material as any).__laserInjected = true
}
...
function scanScene() {
  ...
  scene.traverse((obj) => {
    if ((obj as THREE.Mesh).isMesh || 
        ((obj as any).geometry && (obj as any).material)) {
...
function checkSceneChanges() {
  let currentCount = 0
  scene.traverse((obj) => {
    if ((obj as THREE.Mesh).isMesh || ((obj as any).geometry && (obj as any).material)) {
      currentCount++
    }
  })
